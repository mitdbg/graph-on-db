...
//		}
	...
	/**
	 * Convert an long array to byte array 
	 * @param array
	 * @return
	 */
	public static byte[] toBytes(List<Long> array) {
		byte[] a = new byte[8*array.size()];
		int offset=0;
		for(long val: array){
			append(a, offset, val);
			offset+=8;
	...
	public static List<Long> getLongArray(byte[] a) {
		List<Long> values = Lists.newArrayList();
		for(int off=0;off<a.length;off+=8)
			values.add(getLong(a, off));
		return values;
	}
	...
	public static void main(String[] args) {
		...
		byte[] a = CompressionUtils.compressGZIP(toBytes(list));
		System.out.println(a);
		
		BytesWritable t = new BytesWritable(a);
		System.out.println(t);
		
		BytesWritable t2 = new BytesWritable(t.getBytes());
		List<Long> list2 = getLongArray(CompressionUtils.decompressGZIP(t2.getBytes()));
